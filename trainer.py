...
class Trainer(nn.Module):
    ...
    def validate(self, model, evaluator, val_loader, device):
        ...
        # Print validation metrics to stdout so CPU runs can observe the metric
        # trajectory without the optional Neptune logger.
        avgs = {m: out[f"{split}/{m}/avg"] for m in evaluator.metric_names()}
        print(f"  [val step {self.step:>5}] " + "  ".join(f"{m}={v:.4f}" for m, v in avgs.items()), flush=True)
