...
def extract_tar(fn, unzip_dir):
    ...
    # Use a context manager so the tar handle is closed before unlink on Windows
    # (otherwise os.unlink fails with WinError 32 "file in use").
    ...
    try:
        ...
        # Non-fatal: the extracted files are what matter, not the tar cleanup.
        logging.warning(f"Could not delete {fn}: {e}")